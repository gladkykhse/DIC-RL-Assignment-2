...
class Environment:
    ...
    def _process_state(self) -> list[float]:
        """Converts current environment state into a 5D normalized state."""
        row, col = self.agent_pos
        height, width = self.grid.shape
        norm_row = row / (height - 1)
        norm_col = col / (width - 1)
        delta_row = (row - self.start_pos[0]) / (height - 1)
        delta_col = (col - self.start_pos[1]) / (width - 1)
        remaining_norm = (self.initial_target_count - self.world_stats["total_targets_reached"]) / max(1, self.initial_target_count)
        return [norm_row, norm_col, delta_row, delta_col, remaining_norm]
    ...
    def step(self, action: int) -> tuple[tuple[Any, Any, int | Any], Any, bool, dict]:
        ...
        return self._process_state(), reward, self.terminal_state, self.info
